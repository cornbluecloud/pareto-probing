...
for r in range(0, 12):
    ...
    #   filename = "../reduced_training_data/d0_" + str(p)+"_bert/all_results.tsv"
            filename = "../multiple_bijections/R0" +str(r) +"_d"+ str(d)+"_100_bert/all_results.tsv"
        ...
        if r == 0:
            plt.scatter(x,z, label = "R"+ str(r)+"_"+ str(d) + "% d")
        else:
            plt.scatter(x,z, label = "R"+ str(r)+"_"+ str(d) + "% d", marker = "*")

        # ax.plot(x, y, z, label = "R1_"+ str(d) + "% d")
    #ax = plt.axes(projection='3d')
    # for d in [0]: #range(0, 100, 10):
    #     x, y, z  = [], [], []
    #     for s in [100]: #range(10, 100, 10):
    ...
    #                 y.append(s)
    #                 z.append(float(row[ref[y_axis]]))
    #     plt.scatter(y,z, label = "R0_"+ str(0) + "% dimension removed", marker = "*")
