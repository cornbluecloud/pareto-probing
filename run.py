...
def train_all(index, config_keys, configs):
    ...
    if index >= len(config_keys):
        ...
        curr_template = curr_template.replace("<language>", str(cf['language'])).replace("<training_data>",str(cf['training_data'])).replace("<missing_data>",str(cf['missing_data']))
        curr_template = curr_template.replace("<representation>", str(cf['representation'])).replace("<task>",str(cf['task'])).replace("<representation_type>",str(cf['representation_type']))
        curr_template = curr_template.replace("<model>", str(cf['model']))
...
def main():
    ...
    configs = importlib.import_module("default").config
